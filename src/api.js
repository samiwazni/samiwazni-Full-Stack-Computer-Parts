...
export const handleComponentChangeAdmin = async (
  event,
  type,
  formFields,
  componentId
  ...
  try {
    const response = await fetch(
      `http://localhost:4000/api/${type}/${componentId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
export const handleComponentDeleteAdmin = async (type, id) => {
  ...
  const response = await fetch(
    `http://localhost:4000/api/${type}/${id}/delete`,
    {
      method: "DELETE",
...
export const handleCredentialChangeAdmin = async (
  event,
  newAdmin,
  initialAdmin,
  newBanned,
