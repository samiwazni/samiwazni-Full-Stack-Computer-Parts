  ...
  userDb.get(sql, [userId], (err, user) => {
    if (err) {
      ...
      return res
        .status(200)
        .json({
          userData: { ...userData, isAdmin: isAdmin, isBanned: isBanned },
        });
    }
  });
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      userDb.run(updateQuery, queryParams, function (err) {
        ...
        return res
          .status(200)
          .json({ message: "User updated successfully", id: this.lastID });
      });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ message: "Internal server error" });
...
api.patch("/api/computerwizard", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Price must be a number between 500-5000" });
    } else if (formFields.price < 500 || formFields.price > 5000) {
      return res
        .status(400)
        .json({ message: "Price must be between 500-5000" });
    }
    if (
    ...
    userDb.get(selectQuery, [userId], (err, row) => {
      ...
      let existingData = row.Completed_builds
        ? JSON.parse(row.Completed_builds)
        : [];
      ...
      userDb.run(updateQuery, [combinedData, userId], function (err) {
        ...
        return res
          .status(200)
          .json({
            message: "Completed builds updated successfully",
            id: this.lastID,
          });
      });
    });
  ...
  try {
    ...
    if (typeof Url !== "undefined") {
      if (!regEx.test(Url)) {
        return res
          .status(400)
          .json({ message: "Invalid URL format. Must be a Jimms product." });
      }
    }
          ...
          if (key === "Url") {
            if (!regEx.test(formFields[key])) {
              return res
                .status(400)
                .json({
                  message: "Invalid URL format. Must be a Jimms product.",
                });
            }
          }
          updateQuery += `${key} = ?, `;
    ...
    db.run(updateQuery, queryParams, function (err) {
      ...
      return res
        .status(200)
        .json({ message: "Component updated successfully", id: this.lastID });
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal server error" });
...
api.post("/api/:component", authenticateJWT, (req, res) => {
  ...
  const componentQueries = {
    chassis:
      "INSERT INTO chassis (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Chassis_type, Dimensions, Color, Compatibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    cpu_cooler:
      "INSERT INTO cpu_cooler (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Compatiblilty, Cooling_Potential, Fan_RPM, Noise_Level, Dimensions) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    memory:
      "INSERT INTO memory (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Type, Amount, Speed, Latency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    motherboard:
      "INSERT INTO motherboard (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Chipset, Form_Factor, Memory_Compatibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    psu: "INSERT INTO psu (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Is_ATX12V, Efficiency, Modular, Dimensions) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    storage:
      "INSERT INTO storage (ID, Url, Price, Name, Manufacturer, Image, Image_Url, Capacity, Form_Factor, Interface, Cache, Flash, TBW) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
  };
...
api.delete("/api/:component/:id/delete", authenticateJWT, (req, res) => {
  ...
  db.run(sql, [id], function (err) {
    ...
    return res
      .status(200)
      .json({ message: "Item deleted successfully", id: id });
  });
});
